...
def interview(conf_path):
    ...
    options["hostname"] = input("Internet browser accessible hostname to access this server "
                                "(ex: http://%s): " % socket.gethostname())

    print("\nConfigure Email")
    print("---------------")
    options["smtp_host"] = input("What is host:port for your smtp server (ex: smtphost:25): ").strip()
    smtp_user = input("What is the username should flawless use to access your smtp "
    ...
    options["ignore_vcs_email_domain"] = input("Are all developer emails on %s (y/n): " %
                                               options["email_domain_name"])[0] in ['y', 'Y']
    options["default_contact"] = input("If Flawless can't figure out which developer to email, "
    ...
    if (input("Use %s as the directory to store configuration information (y/n)? " % conf_dir_path).strip()[0]
            in ['n', 'N']):
        ...
        if not os.path.exists(conf_dir_path):
            ...
            copy_files = input("Overwrite existing files in %s (y/n)? " % conf_dir_path).strip()[0] in ['y', 'Y']
    ...
    options["data_dir_path"] = input("Enter path to directory were Flawless can persist "
    ...
    if input("Should Flawless checkout a copy of your repo (y/n): ")[0] in ['y', 'Y']:
        ...
        if input("Do you cut release branches for the code flawless will monitor (y/n)? ")[0] in ['y', 'Y']:
